...
namespace Photon.Pun.Demo
{
    ...
    public class WebRpcImplementationExample : MonoBehaviour, IWebRpcCallback
    {
        ...
        public void OnWebRpcResponse(OperationResponse response)
        {
            ...
            switch (response.ReturnCode)
            {
                case ErrorCode.Ok:
                    ...
                    else if (webRpcResponse.ResultCode == -1)
                    {
                        Debug.LogErrorFormat(
                            "Web server did not return ResultCode for WebRPC method=\"{0}\", Message={1}",
                            webRpcResponse.Name, webRpcResponse.Message);
                    }
                    else
                    {
                        Debug.LogErrorFormat(
                            "Web server returned ResultCode={0} for WebRPC method=\"{1}\", Message={2}",
                ...
                case ErrorCode.ExternalHttpCallFailed: // web service unreachable
                    Debug.LogErrorFormat("WebRPC call failed as request could not be sent to the server. {0}",
                ...
                case ErrorCode.InvalidOperation
                    : // WebRPC not configured at all OR not configured properly OR trying to send on name server
                    ...
                    {
                        Debug.LogErrorFormat("WebRPC not properly configured or not configured at all. {0}",
                            response.DebugMessage);
                    }
        ...
        public static void WebRpcCall(string methodName, object parameters = null, bool sendAuthCookieIfAny = false)
        {
            ...
            {
                Debug.LogErrorFormat(
                    "Error sending WebRPC \"{0}\" (\"{1}\") request, check the previous error logs for more details",
